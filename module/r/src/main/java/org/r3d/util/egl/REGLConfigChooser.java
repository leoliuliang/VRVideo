 ...
 */
public class REGLConfigChooser implements GLSurfaceView.EGLConfigChooser {
    ...
    private final IRSurface.ANTI_ALIASING_CONFIG mAntiAliasingConfig;

    public REGLConfigChooser(int glMajorVersion, @NonNull IRSurface.ANTI_ALIASING_CONFIG antiAliasingConfig,
                             int sampleCount, int bitsRed, int bitsGreen, int bitsBlue, int bitsAlpha, int bitsDepth) {
        mAntiAliasingConfig = antiAliasingConfig;

        if (mAntiAliasingConfig.equals(IRSurface.ANTI_ALIASING_CONFIG.MULTISAMPLING)) {
            mConfigSpec = new int[]{
                ...
                antiAliasingConfig.equals(IRSurface.ANTI_ALIASING_CONFIG.MULTISAMPLING) ? 1 : 0, /* Do we use sample buffers */
                EGL10.EGL_SAMPLES,
                antiAliasingConfig.equals(IRSurface.ANTI_ALIASING_CONFIG.MULTISAMPLING) ? sampleCount : 0, /* Sample count */
                EGL10.EGL_NONE
            };
        } else if (mAntiAliasingConfig.equals(IRSurface.ANTI_ALIASING_CONFIG.COVERAGE)) {
            mConfigSpec = new int[]{
                EGL10.EGL_RED_SIZE, bitsRed,
                EGL10.EGL_GREEN_SIZE, bitsGreen,
                EGL10.EGL_BLUE_SIZE, bitsBlue,
    ...
    public IRSurface.ANTI_ALIASING_CONFIG getAntiAliasingConfig() {
