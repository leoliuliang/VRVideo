 ...
 */
public class RTextureView extends TextureView implements IRSurface {
    ...
    private final WeakReference<RTextureView> mThisWeakRef = new WeakReference<>(this);
    ...
    public RTextureView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        applyAttributes(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public RTextureView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void applyAttributes(Context context, AttributeSet attrs) {
        ...
        final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.RTextureView);
        final int count = array.getIndexCount();
        for (int i = 0; i < count; ++i) {
            int attr = array.getIndex(i);
            if (attr == R.styleable.RTextureView_frameRate) {
                mFrameRate = array.getFloat(attr, 60.0f);
            } else if (attr == R.styleable.RTextureView_renderMode) {
                mRenderMode = array.getInt(attr, RENDERMODE_WHEN_DIRTY);
            } else if (attr == R.styleable.RTextureView_antiAliasingType) {
                mAntiAliasingConfig = ANTI_ALIASING_CONFIG.fromInteger(array.getInteger(attr, ANTI_ALIASING_CONFIG.NONE.ordinal()));
            } else if (attr == R.styleable.RTextureView_bitsRed) {
                mBitsRed = array.getInteger(attr, 5);
            } else if (attr == R.styleable.RTextureView_bitsGreen) {
                mBitsGreen = array.getInteger(attr, 6);
            } else if (attr == R.styleable.RTextureView_bitsBlue) {
                mBitsBlue = array.getInteger(attr, 5);
            } else if (attr == R.styleable.RTextureView_bitsAlpha) {
                mBitsAlpha = array.getInteger(attr, 0);
            } else if (attr == R.styleable.RTextureView_bitsDepth) {
    ...
    public void setSurfaceRenderer(IRSurfaceRenderer renderer) throws IllegalStateException {
        ...
        // Create our delegate
        final RendererDelegate delegate = new RTextureView.RendererDelegate(renderer, this);
     ...
     * <p>If this method is
     * called, it must be called before {@link #setSurfaceRenderer(IRSurfaceRenderer)}
     * is called.
     * <p/>
     ...
     * <p>If this method is called, it must be called before {@link #setSurfaceRenderer(IRSurfaceRenderer)}
     * is called.
     ...
     * <p/>
     * This method can only be called after {@link #setSurfaceRenderer(IRSurfaceRenderer)}
     *
    ...
    private static class RendererDelegate implements SurfaceTextureListener {
        ...
        final RTextureView mRTextureView;
        final IRSurfaceRenderer mRenderer;

        public RendererDelegate(IRSurfaceRenderer renderer, RTextureView textureView) {
            mRenderer = renderer;
            mRTextureView = textureView;
            mRenderer.setFrameRate(mRTextureView.mRenderMode == IRSurface.RENDERMODE_WHEN_DIRTY ?
                mRTextureView.mFrameRate : 0);
            mRenderer.setAntiAliasingMode(mRTextureView.mAntiAliasingConfig);
            mRenderer.setRenderSurface(mRTextureView);
            mRTextureView.setSurfaceTextureListener(this);
        }

        @Override
        public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
            mRTextureView.surfaceCreated(width, height);
        }

        @Override
        public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
            mRTextureView.surfaceChanged(width, height);
        }

        @Override
        public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
            mRTextureView.setCleanupTexture(surface);
            mRTextureView.surfaceDestroyed();
            return false;
        }
    ...
    private static class EglHelper {
        private WeakReference<RTextureView> mRajawaliTextureViewWeakRef;
        EGL10 mEgl;
        ...
        public EglHelper(WeakReference<RTextureView> glSurfaceViewWeakRef) {
        ...
        public void start() {
            ...
            RTextureView view = mRajawaliTextureViewWeakRef.get();
            if (view == null) {
                mEglConfig = null;
                mEglContext = null;
            } else {
    ...
    static class GLThread extends Thread {
        ...
        GLThread(WeakReference<RTextureView> glSurfaceViewWeakRef) {
            super();
            mWidth = 0;
            mHeight = 0;
            mRequestRender = true;
